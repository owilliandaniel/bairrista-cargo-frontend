...
function SimularPreco({ onClose }) {
  ...
  const handleSimulacaoSubmit = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Erro ao calcular o preço. Tente novamente.');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleSolicitarServico = async () => {
    ...
    try {
      ...
      const MAP_TIPO_MUDANCA = {
        'RESIDENCIAL': 'RES',
        'COMERCIAL': 'COM',
        'OUTROS': 'OUT'
      }
      const MAP_TIPO_LOCAL = {
        'CASA': 'CAS',
        'APARTAMENTO': 'APT',
        'OUTROS': 'OUT'
      }
      ...
      const errorMsg = err.response?.data?.detail || err.response?.data?.message || 'Erro ao solicitar serviço. Tente novamente.'
      ...
      {!resultado ? (
        <>
          ...
          <div>
            ...
            <select 
              ...
              style={{ 
                ...
                width: '100%',
                fontSize: '1.1rem',
                background: '#1a1a1a',  // ← MUDADO de rgba para cor sólida
                color: '#fff',
                transition: 'all 0.3s ease',
                fontWeight: '300',
                cursor: 'pointer',
                appearance: 'auto',  // ← ADICIONADO
                WebkitAppearance: 'menulist',  // ← ADICIONADO
                MozAppearance: 'menulist'  // ← ADICIONADO
              }}
              onFocus={(e) => {
                e.currentTarget.style.border = '1px solid rgba(255,255,255,0.5)';
                e.currentTarget.style.background = '#2a2a2a';  // ← MUDADO
              }}
              onBlur={(e) => {
                e.currentTarget.style.border = '1px solid rgba(255,255,255,0.2)';
                e.currentTarget.style.background = '#1a1a1a';  // ← MUDADO
              }}
            >
          ...
          <div style={{ marginTop: '1rem' }}>
            <InventarioIA 
              onItensDetectados={(itens) => setItensInventario(itens)}
            />
          </div>
